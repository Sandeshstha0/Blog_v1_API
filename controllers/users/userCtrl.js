...
const blockUserCtrl = async (req, res, next) => {
  try {
    ...
    if (userWhoBlocked && userToBeBlocked) {
      ...
      res.json({
        status: "success",
        data: "You have successively blocked the user",
      });
    }
...
const unblockUserCtrl = async (req, res, next) => {
  try {
    ...
    //check if userToBeUnBlocked and UserWhoUnblock are found
...
const deleteUserAccountCtrl = async (req, res, next) => {
  try {
    ...
    //4. Delete all category of the users
...
const profilPhotoUploadCtrl = async (req, res, next) => {
  try {
    ...
    //4 check if a user is up dating their photo
    if (req.file) {
      //5 udate profile photo
      await User.findByIdAndUpdate(
        req.userAuth,
        {
          $set: {
      ...
      res.json({
        staus: "success",
        data: "you have succesfully updated profile photo",
      });
    }
  } catch (error) {
